...
func TestAuditUpgradeV030RealVolumeReadableThroughBothSurfaces(t *testing.T) {
	...
	if v, err := st.GetKV(ctx, "schema_version"); err != nil || v != "9" {
...
func TestAuditInterruptedMigrationReplayPreservesData(t *testing.T) {
	...
	if v, err := reopened.GetKV(ctx, "schema_version"); err != nil || v != "9" {
		t.Fatalf("schema_version after replay = %q, %v; want repaired to 9", v, err)
	}
...
func TestAuditSchemaVersionFailClosedMessageAndCorruptValue(t *testing.T) {
	t.Run("future version names both numbers", func(t *testing.T) {
		...
		legacy := buildV030Database(t, path)
		if _, err := legacy.Exec(`UPDATE kv SET value='10' WHERE key='schema_version'`); err != nil {
			t.Fatal(err)
		}
		if err := legacy.Close(); err != nil {
			t.Fatal(err)
		...
		_, err := Open(path)
		if err == nil {
			t.Fatal("Open on schema_version 10 succeeded")
		}
		for _, needle := range []string{"10", "9", "newer than this binary supports"} {
...
func TestAuditConcurrentOpenSameFile(t *testing.T) {
	for round := 0; round < 3; round++ {
		...
		if v, err := st.GetKV(context.Background(), "schema_version"); err != nil || v != "9" {
			st.Close()
			t.Fatalf("round %d: schema_version = %q, %v; want 9", round, v, err)
		}
...
func TestAuditDowngradeV04DatabaseUnderV03OpenSemantics(t *testing.T) {
	...
	var v string
	if err = v03.QueryRow(`SELECT value FROM kv WHERE key='schema_version'`).Scan(&v); err != nil || v != "9" {
