...
func TestMigration004AddsNullablePalPlacementColumns(t *testing.T) {
	...
	defer st.Close()
	if v, getErr := st.GetKV(context.Background(), "schema_version"); getErr != nil || v != "12" {
		t.Fatalf("schema_version = %q, %v; want 12", v, getErr)
	}
...
func TestUpgradeFromV030SchemaAppliesAPIKeysMigration(t *testing.T) {
	...
	if err != nil || v != "12" {
		t.Fatalf("schema_version after upgrade = %q, %v; want 12", v, err)
	}
	...
	if v, err = reopened.GetKV(ctx, "schema_version"); err != nil || v != "12" {
		t.Fatalf("schema_version after no-op reopen = %q, %v; want 12", v, err)
	}
}
